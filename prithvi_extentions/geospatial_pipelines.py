...
class DeleteBands(object):
    ...
    def __call__(self, results):
        if self.bands is not None:
            for b in self.bands:
                # replace specific tensors with zeros
                results["img"][..., b] = torch.zeros(results["img"][..., b].shape)
        
        return results
    
@PIPELINES.register_module()
class DuplicateBands(object):
    ...
    def __call__(self, results):
        if self.bands is not None:
            for b in self.bands:
                # replace specific tensors with duplicates
                results["img"][..., b] = results["img"][..., b-len(self.bands)]
...
@PIPELINES.register_module()
class TorchCenterCrop(object):

    """

    It crops a multichannel tensor at the center.
    ...
    def __call__(self, results):
        croptransform = transforms.CenterCrop(self.crop_size)
        results["img"] = croptransform(results["img"]).float()
        results["gt_semantic_seg"] = croptransform(results["gt_semantic_seg"])
